...
interface UpdateAssociateDialogFormProps {
  children: ReactNode;
  associateId: string;
}
...
export function UpdateAssociateDialogForm({
  associateId,
  children,
}: UpdateAssociateDialogFormProps) {
  const queryClient = useQueryClient();
  const [currentAssociated, setCurrentAssociated] = useState<
    string | undefined
  >();
  ...
  const { data: associateData, isLoading } = useAssociateById(currentAssociated);

  function safeFormat(value: string | null | undefined, mask: string): string {
    if (!value) return "";
    try {
      return format(value, {
        mask,
      ...
      console.warn("Erro ao aplicar máscara:", e, { value, mask });
      return value;
    }
  }

  function mapApiToFormValues(apiData: any): CreateAssociateFormValues {
    const address = apiData?.associate?.AssociateAddress?.[0];

    return {
      email: apiData?.email || "",
      name: apiData?.name || "",
      cpf: safeFormat(apiData?.cpf, "___.___.___-__"),
      phone: safeFormat(apiData?.phone, "(__) _____-____"),
      rg: apiData?.rg || "",
      birthday: apiData?.birthday ? new Date(apiData.birthday) : undefined,
      profession_name: apiData?.profession_name || "",
      street: address?.street || "",
      city: address?.city || "",
      state_id: address?.country_state?.id?.toString() || "",
      zip_code: safeFormat(address?.zip_code, "_____-___"),
      number: address?.number || "",
      neighborhood: address?.neighborhood || "",
      membership_date: apiData?.associate?.membership_date
        ? new Date(apiData.associate.membership_date)
        : undefined,
      payment_due_date: apiData?.associate?.payment_due_date?.toString() || "",
      payment_method_preference_id:
        apiData?.associate?.paymentPreferenceMethod?.id || "",
      associate_plan_id: apiData?.associate?.associatePlan?.id || "",
      associate_status_id:
        apiData?.associate?.associateStatus?.id?.toString() || "",
      image_path: apiData?.image_path || "",
      color: apiData?.color || "",
    };
  }
  ...
  const {
    ...
    setError,
    formState: { errors },
  } = useForm<CreateAssociateFormValues>({
  ...
  if (numericCEP?.length === 8) {
    (async () => {
      try {
        ...
        const res = await getAddressByCEP({ cep: numericCEP });
        if (res?.data) {
          setValue("street", res.data.logradouro || "");
          setValue("neighborhood", res.data.bairro || "");
          setValue("city", res.data.localidade || "");
          const stateOption = countryStates?.find(
            (s: { initials: string | undefined }) => s.initials === res.data?.uf
          );
...
}, [cepValue, countryStates, setValue]);

useEffect(() => {
  if (associateData) {
    const mappedValues = mapApiToFormValues(associateData);
    reset(mappedValues);
  }
}, [associateData, reset]);
  ...
  return (
    ...
    >
      <TooltipComponent title="Atualizar dados do associado">
      ...
      >
        {isLoading ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <DialogHeader>
              <DialogTitle>Atualizar Associado</DialogTitle>
              <DialogDescription>
          ...
          <div className="w-full flex items-center justify-center">
            <InputFile
              ...
              initialImage={associateData?.image_path}
            />
          </div>
          ...
          <div className="space-y-3 rounded-lg border p-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <ControlledMaskedInput loading={zipcodeIsLoading} control={control} name="zip_code" label="CEP" mask="_____-___" placeholder="00000-000" rules={{ required: "CEP é obrigatório" }} />
